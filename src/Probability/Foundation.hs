{-# OPTIONS_GHC -funfolding-use-threshold=200 -fexpose-all-unfoldings -fspecialize-aggressively -Wno-missing-signatures #-} -- inline as much as possible
{-# LANGUAGE GADTs #-}
{-# LANGUAGE RankNTypes #-}
...
-- | Sample is based on State StdGen, and is pure.
...
normal :: (MonadUniform m, UniformFloat k, Ord k) => Mu k -> Sigma k -> m k
normal mu sigma = fmap ((+mu) . (*sigma)) $ do
  u1 <- max 1e-323 <$> getUniformR (0, 1)
  u2 <- getUniformR (0, 1)
...
poisson lambda
  ...
  | lambda > 10000 = do
    ...
    if x < 0 then poisson lambda else return $ round x
  | otherwise = error "poisson : lambda must be non-negative"
  where poissonList = (\(lp, k) -> (exp lp, k)) <$> iterate nextTerm (-lambda, 0)
        nextTerm (p, k) = (p + log lambda - log (fromIntegral k+1), k+1)

-- | Sample from a gamma distribution with shape parameter alpha and rate parameter beta.
