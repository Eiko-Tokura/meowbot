-- | Core database types, small in size
...
-- | Generated by admin command, or other one-off costs
OneOffCostRecord
  walletId    WalletId
  cost        Amount             -- cost to user, should be positive
  actualCost  ActualCost Maybe   -- actual cost to us, so the difference is our profit/loss. Nothing = 0
  time        UTCTime
  handlerId   UserId     Maybe
  description Text       Maybe
...
BotSetting -- Overlappable by BotSettingPerChat
  ...
  withTimeStampTimezone   Int                  Maybe
  sourceOnStartup         Bool                 Maybe
