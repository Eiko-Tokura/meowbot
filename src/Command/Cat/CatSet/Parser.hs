...
data BotSettingItem
  ...
  | MultiResponse           (Maybe Bool)
  | WithTimeStamp           (Maybe Bool)
...
catSetParser =
  (MP.headCommand "cat-" >> do
    ...
    asum
      ...
      , MP.string "multiResponse"           >> fmap (action range) (MultiResponse           <$> MP.optMaybe (MP.spaces >> MP.bool))
      , MP.string "withTimeStamp"           >> fmap (action range) (WithTimeStamp           <$> MP.optMaybe (MP.spaces >> MP.bool))
      , MP.string "activeProbability"       >> fmap (action range) (ActiveProbability       <$> MP.optMaybe (MP.spaces >> require (\x -> x <= 0.20 && x >= 0) MP.nFloat))
      , MP.string "maxMessageInState"       >> fmap (action range) (MaxMessageInState       <$> MP.optMaybe (MP.spaces >> MP.intRange 1 40))
