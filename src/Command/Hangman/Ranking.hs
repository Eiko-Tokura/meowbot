...
updateTotalPP uid nick = do
  listScores <- runDB $ selectList [HangmanRecordUserId ==. uid] []
  let totalPP = computePP (fromMaybe 0 . hangmanRecordScore . entityVal <$> listScores)
  rank <- fmap (+1) . runDB $ count [HangmanRecordScore >. Just totalPP, HangmanRecordUserId !=. uid]
...
-- the total pp it the maximal 50 consecutive sum of the scores
