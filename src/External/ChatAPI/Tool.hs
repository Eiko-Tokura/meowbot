...
appendToolPrompts clist sep
  ...
  | otherwise =
    ( T.unlines
      ...
      , ""
      , "* Tool call output should NOT be mixed with other text. If you really want to mix other output with tool call, you MUST:"
      , "  - use a code block (```json ... ```) to wrap it"
      , "  - or put the tool call at the end of the output."
      , ""
      , "  otherwise the tool call will fail to parse."
      , ""
